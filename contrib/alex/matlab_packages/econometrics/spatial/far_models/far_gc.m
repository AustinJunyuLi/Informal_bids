function results = far_gc(y,W,ndraw,nomit,prior)
...
%----------------------------------------------------------------
% USAGE: result =  far_gc(y,W,ndraw,nomit,prior)
...
%----------------------------------------------------------------
% SEE ALSO: (far_gcd, far_gcd2 demos), far_g.m, prt
% --------------------------------------------------------------
...
if nargin > 3, % some basic error checking
 [n n2] = size(W);  
 if n ~= n2 % a non-square spatial weight matrix
 error('far_gc: Wrong size 1st-order contiguity matrix');
 end; 
 [tst junk] = size(y);
 if tst ~= n % y-vector length doesn't match W-matrix
 error('far_gc: Wrong size y vector on input');
 end;
...
[nu,d0,rval,mm,kk,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,seedflag,seed,dflag] = far_parse(prior);
...
fprintf(' -- far_gc: MCMC sampling -- \n');
if seedflag == 0
time3 = clock; % start timing the sampler
[rout,sout,rdraw,vmean] = ...
 far_gcc(y,Wy,detval(:,2),detval(:,1),n,length(detval),rval,ndraw,nomit,nu,d0,mm,kk);
time3 = etime(clock,time3);
else
time3 = clock; % start timing the sampler
[rout,sout,rdraw,vmean] = ...
 far_gcc(y,Wy,detval(:,2),detval(:,1),n,length(detval),rval,ndraw,nomit,nu,d0,mm,kk,rseed);
time3 = etime(clock,time3);
end;
...
function [nu,d0,rval,mm,kk,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,seedflag,seed,dflag] = far_parse(prior)
...
if nf > 0
 for i=1:nf
    ...
    elseif strcmp(fields{i},'lflag')
        ...
        error('far_gc: unrecognizable lflag value on input');
        end;
