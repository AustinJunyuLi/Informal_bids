...
options = zeros(1,18); % options for fmin (simplex optimization)
options(1,1) = 0; 
options(1,2) = 1.e-4; 
options(1,14) = 500;
rflag = 0;
...
opti.pflag = 0;
 ...
 for i=1:nf
    if strcmp(fields{i},'parm')
       bstart = 1; parm = info.parm;
    elseif strcmp(fields{i},'convg')
       opti.ftol = info.convg;
    elseif strcmp(fields{i},'maxit')
        opti.maxit = info.maxit;
    elseif strcmp(fields{i},'method')
        opti.meth = info.method;
    elseif strcmp(fields{i},'pflag')
        opti.pflag = info.pflag;
        options(1,1) = info.pflag;
    elseif strcmp(fields{i},'lflag')
        ...
        elseif tst == 1,
        ldetflag = 1; 
        lmin = 0; lmax = 1; rflag = 1; % reset this from default
        elseif tst == 2,
        ldetflag = 2; 
        lmin = 0; lmax = 1; rflag = 1; % reset this from default
        else
...
if exitflag == 0 
fprintf(1,'sac: convergence not obtained in %4d iterations \n',output.iterations);
end;
...
elseif n > 500 % use numerical hessian
...
if ldetflag == 0
hessn = hessian('f2_sac',parm,y,x,W1,W2);
else
