...
if nargin == 8   % parse input values
 ...
 for i=1:nf
    ...
    elseif strcmp(fields{i},'lflag')
        tst = prior.lflag;
        if tst == 0,
        lflag = 0;          elseif tst == 1,
        lflag = 1; rmin = 0; rmax = 1;  % reset this from default
		eflag = 1;
        elseif tst == 2,
        lflag = 2; rmin = 0; rmax = 1;  % reset this from default
		eflag = 1;
        else
...
if pflag == 1
if ~isscalar(prho)
error('semip_g: prior mean for rho should be a scalar');
end;
if ~isscalar(pvar)
error('semip_g: prior variance for rho should be a scalar');
end;
end;
