...
if n1 ~= n
error('sdmt_g: x-matrix contains wrong # of observations');
elseif n3 ~= n4
error('sdmt_g: W matrix is not square');
elseif n3~= n
error('sdmt_g: W matrix is not the same size at y,x');
end;
...
[nu,d0,rval,mm,kk,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,novi_flag,cflag,vflag] = sdm_parse(prior);
...
otherwise
error('sdmt_g: unrecognized prior.novi value on input');
...
function [nu,d0,rval,mm,kk,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,novi_flag,cflag,vflag] = sdm_parse(prior)
