...
hwait = waitbar(0,'sdm\_g: MCMC sampling ...');
t0 = clock;                  
iter = 1;
...
% integrate out rho up front to speed things up
          AI = inv(xdx'*xdx);
...
% trapezoid rule
isum = sum((yy(2:nrho,1) + yy(1:nrho-1,1)).*(xx(2:nrho,1) - xx(1:nrho-1,1))/2);
...
otherwise
error('sdm_g: unrecognized prior.novi value on input');
...
elseif ldetflag == -1 % the user fed down a detval matrix
    time1 = 0;
        ...
        if detval == 0
            error('sdm_g: wrgon lndet input argument');
        end;
        [n1,n2] = size(detval);
        if n2 ~= 2
