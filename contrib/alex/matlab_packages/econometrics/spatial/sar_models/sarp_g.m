...
% --------------------------------------------------------------
% SEE ALSO: (sarp_gd, sarp_gd2 demos) sarp_gc.m, prt
% --------------------------------------------------------------
...
[nu,d0,rval,mm,kk,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,c,T,prior_beta,cc,metflag,novi_flag] = sar_parse(prior,k);
...
iter = 1;
     ...
     if metflag == 1
         % metropolis step to get rho update
          ...
          mu = xb + rho*Wx1*bhat + (rho.^2)*Wx2*bhat + (rho.^3)*Wx3*bhat + ...
          ...
          ym = mu + C;
	         ...
	         tmp1 = normrt_rnd(ym(zipv,1),yvar(zipv,1),0);
	         tmp2 = normlt_rnd(ym(zipo,1),yvar(zipo,1),0);
     ...
     if metflag == 1
         % metropolis step to get rho update
          ...
          ym = mu + C;
	         ...
	         y(zipv,1) = normrt_rnd(ym(zipv,1),yvar(zipv,1),0);
	         y(zipo,1) = normlt_rnd(ym(zipo,1),yvar(zipo,1),0);
...
function [nu,d0,rval,mm,kk,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,c,T,prior_beta,cc,metflag,novi_flag] = sar_parse(prior,k)
