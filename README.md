...
> and this branch is for implementing the Jan 14, 2026 Task B two-stage simulation/estimation
> (in progress). The naive baseline lives on branch `basic`.
...
This branch uses the selection-aware MH-within-Gibbs baseline for Task A and is the
workstream for implementing the Jan 14, 2026 two-stage simulation/estimation task. It
provides Task A (single cutoff) and Task B (type-specific cutoffs) simulation
infrastructure and diagnostics for evaluating estimator performance.
